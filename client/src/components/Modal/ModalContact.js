...
function ModalContact ( {isOpen, setIsOpen, text, photo, walker, confirmHandler} ) {
    ...
    <StyledModal className="modal" onClick={openModalHandler}>
        <StyledModalCon onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
                <div>
                    <p><img src={photo} alt="" className="img-circle"/></p>
                    <strong>{walker} 님</strong>
                    <b>{text}</b>
                </div>
            </div>
            <StyledModalFooter className="modal-footer">
                <button type="button" className="btn-modal con" onClick={()=>confirmHandler()}>전화 걸기</button>
            </StyledModalFooter>
        </StyledModalCon>
    </StyledModal> : null}
